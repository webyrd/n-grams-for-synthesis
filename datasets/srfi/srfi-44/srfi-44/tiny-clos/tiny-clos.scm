...
(letrec ((instances '())
	 (get-vector
	  (lambda (closure)
            (cond ((assq closure instances) => cdr)
                  (else #f)))))
  ...
  (set! %instance?
        (lambda (x) (and (get-vector x) #t)))

  (set! %instance-class
	(lambda (closure)
          (cond ((get-vector closure)
                 => (lambda (vec) (vector-ref vec 2)))
                (else
                 (error "Can't get class of non-instance" closure)))))

  (set! %set-instance-class-to-self
	(lambda (closure)
          (cond ((get-vector closure)
                 => (lambda (vec) (vector-set! vec 2 closure)))
                (else
                 (error "Can't set instance class of non-instance" closure)))))

  (set! %set-instance-proc!
        (lambda (closure proc)
	  (let ((vector (get-vector closure)))
	    (if (vector-ref vector 1)
		(error "Can't set procedure of instance." closure proc)
		(vector-set! vector 0 proc)))))

  (set! %instance-ref
        (lambda (closure index)
          (cond ((get-vector closure)
                 => (lambda (vec) (vector-ref vec (+ index 3))))
                (else
                 (error "Can't %INSTANCE-REF non-instance" closure index)))))

  (set! %instance-set!
        (lambda (closure index new-value)
          (cond ((get-vector closure)
                 => (lambda (vec) (vector-set! vec (+ index 3) new-value)))
                (else
                 (error "Can't %INSTANCE-SET! non-instance"
                        closure index new-value)))))
  )
	...
	(lambda (args)
	  (let ((applicable
		 (collect-if (lambda (method)
			       ...
			       ;      (method-specializers method)
			       ;      args)
                               ; [Riastradh: That caused bugs: what if the
                               ;  argument list was longer than the specs list?
                               ;  So here's the fix.]
